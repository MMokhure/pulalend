...
export default function AdminWithdrawalsPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/withdrawals?userId=${user.id}`);
  ...
  const handleApprove = async (id: number, action: string, notes: string) => {
    try {
      const res = await fetch('/api/admin/withdrawals/approve', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ withdrawalId: id, action, notes, userId: user.id })
      });
      const data = await res.json();
            ...
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {withdrawals.map((w: any) => (
                  <tr key={w.id}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{w.lender_name}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">P{w.amount?.toLocaleString()}</td>
                      ...
                      {w.status === 'pending' && (
                        <div className="flex gap-2">
                          <button 
                            ...
                            onClick={() => handleApprove(w.id, 'approve', '')}
                          >
                            Approve
                          </button>
                          <button 
                            ...
                            onClick={() => {
                              ...
                              if (notes) handleApprove(w.id, 'reject', notes);
                            }}
                          >
                            Reject
                          </button>
