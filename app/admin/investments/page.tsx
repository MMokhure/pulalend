...
export default function AdminInvestmentsPage() {
  ...
  const [view, setView] = useState<"list" | "lenders">("list");
  ...
  const loadInvestments = async () => {
    setLoading(true);
    try {
      let url = `/api/admin/investments?userId=${user.id}`;
    ...
    <DashboardLayout navItems={navItems} userType="admin">
      <div className="p-6">
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">Monitor all platform investments and lender activity</p>
        </div>
        ...
        {summary && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <div className="bg-white shadow rounded-lg p-6">
              ...
              <p className="text-3xl font-bold text-blue-600">P{summary.totalInvested.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
              <p className="text-xs text-gray-500 mt-2">{summary.totalInvestments} investments</p>
            </div>
            <div className="bg-white shadow rounded-lg p-6">
              <h3 className="text-sm font-medium text-gray-500 mb-2">Expected Returns</h3>
              <p className="text-3xl font-bold text-green-600">P{summary.totalExpectedReturn.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
              <p className="text-xs text-gray-500 mt-2">Avg: P{summary.averageInvestment.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
            </div>
            <div className="bg-white shadow rounded-lg p-6">
              <h3 className="text-sm font-medium text-gray-500 mb-2">Platform Commission</h3>
              <p className="text-3xl font-bold text-purple-600">P{summary.totalCommission.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
              <p className="text-xs text-gray-500 mt-2">2% of investments</p>
            </div>
            <div className="bg-white shadow rounded-lg p-6">
              <h3 className="text-sm font-medium text-gray-500 mb-2">Active Investors</h3>
              <p className="text-3xl font-bold text-orange-600">{summary.uniqueLenders}</p>
              <p className="text-xs text-gray-500 mt-2">{summary.uniqueLoans} loans funded</p>
            </div>
          </div>
        )}
        ...
        <div className="mb-6 bg-white shadow rounded-lg p-4">
          <div className="flex gap-4 items-center">
            <div className="flex gap-2">
              <button onClick={() => setView("list")} className={`px-4 py-2 rounded-lg ${view === "list" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700"}`}>
                All Investments
              </button>
              <button onClick={() => setView("lenders")} className={`px-4 py-2 rounded-lg ${view === "lenders" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700"}`}>
                Top Lenders
              </button>
            </div>
            {view === "list" && (
                  ...
                  <tr key={inv.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-sm font-medium text-gray-900">{inv.loanNumber}</span>
                        <span className="text-xs text-gray-500">P{inv.loanAmount.toLocaleString()} @ {inv.interestRate}%</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-sm font-medium text-gray-900">{inv.lender.name}</span>
                        <span className="text-xs text-gray-500">{inv.lender.email}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-sm font-medium text-gray-900">{inv.borrower.name}</span>
                        <span className="text-xs text-gray-500">{inv.borrower.businessName || inv.borrower.email}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900">
                      P{inv.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                    </td>
                    <td className="px-6 py-4 text-sm font-semibold text-green-600">
                      P{inv.expectedReturn.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                    </td>
                    <td className="px-6 py-4 text-sm font-semibold text-purple-600">
                      P{inv.platformCommission.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500">{new Date(inv.investedAt).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Commission Paid</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {topLenders.map((lender, index) => (
                  <tr key={lender.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-900">{lender.investmentCount}</td>
                    <td className="px-6 py-4 text-sm font-semibold text-blue-600">
                      P{lender.totalInvested.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                    </td>
                    <td className="px-6 py-4 text-sm font-semibold text-green-600">
                      P{lender.totalExpectedReturn.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                    </td>
                    <td className="px-6 py-4 text-sm font-semibold text-purple-600">
                      P{lender.totalCommissionPaid.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                    </td>
                  </tr>
                ))}
              </tbody>
