...
export default function AdminTransactionsPage() {
  ...
  }, [user, typeFilter, statusFilter, startDate, endDate]);

  const loadTransactions = async () => {
    setLoading(true);
    try {
      let url = `/api/admin/transactions?userId=${user.id}`;
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      ...
      case "repayment": return "bg-purple-100 text-purple-700";
  ...
  return (
    <DashboardLayout navItems={navItems} userType="admin" title="Transaction Management">
      <div className="p-6">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900">Transaction Management</h1>
          <p className="text-gray-600 mt-2">Monitor all platform financial transactions</p>
        </div>
        ...
        {summary && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            ...
            <div className="bg-white shadow rounded-lg p-6">
              ...
              <p className="text-3xl font-bold text-green-600">P{summary.totalDeposits.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
            </div>
            <div className="bg-white shadow rounded-lg p-6">
              <h3 className="text-sm font-medium text-gray-500 mb-2">Total Investments</h3>
              <p className="text-3xl font-bold text-blue-600">P{summary.totalInvestments.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
            </div>
            <div className="bg-white shadow rounded-lg p-6">
              <h3 className="text-sm font-medium text-gray-500 mb-2">Total Withdrawals</h3>
              <p className="text-3xl font-bold text-red-600">P{summary.totalWithdrawals.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
            </div>
          </div>
        )}
                  ...
                  <tr key={txn.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-sm font-medium text-gray-900">{txn.userName}</span>
                        <span className="text-xs text-gray-500">{txn.email}</span>
                        <span className="text-xs text-gray-400">{txn.userType}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getTypeColor(txn.transactionType)}`}>
                        {txn.transactionType}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900">
                      P{txn.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600 max-w-xs truncate">{txn.description || "N/A"}</td>
